...
package twitter.api {
	...
	public class Twitter extends EventDispatcher
	{
		...
		private static const SUPPRESS_RESPONSE_CODES:String = "suppress_response_codes=true";
      ...
      if(this.source) variables.source = this.source;
		...
		*/
		...
		public function verify(suppressResponseCodes:Boolean=false):void {
			var verifyLoader:URLLoader = this.getLoader(VERIFY);
			
			var url:String = (!suppressResponseCodes) ? VERIFY_URL :   
			                   VERIFY_URL + '?' + SUPPRESS_RESPONSE_CODES;
			verifyLoader.load(twitterRequest(url));
		}
